...
public enum RenderUtil {
	...
	public static void drawFullscreenImage(ResourceLocation image) {
		...
		Gui.drawModalRectWithCustomSizedTexture(0, 0, 0.0F, 0.0F, scaledResolution.getScaledWidth(),
				scaledResolution.getScaledHeight(), (float) scaledResolution.getScaledWidth(),
	...
	public static void drawPlayerHead(String playerName, int x, int y, int width, int height) {
		...
		while (var6.hasNext()) {
			...
			if (player instanceof EntityPlayer) {
				...
				if (playerName.equalsIgnoreCase(ply.getDisplayName().getFormattedText())) {
					GameProfile profile = new GameProfile(ply.getUniqueID(), ply.getDisplayName().getFormattedText());
					...
					Gui.drawModalRectWithCustomSizedTexture(x, y, 0.0F, 0.0F, width, height, (float) width,
							(float) height);
					GL11.glDepthMask(true);
					GL11.glDisable(3042);
					GL11.glEnable(2929);
	...
	public static String getShaderCode(InputStreamReader file) {
		...
		try {
			...
			for (reader = new BufferedReader(
					file); (e = reader.readLine()) != null; shaderSource = String.valueOf(shaderSource) + e + "\n") {
				;
			}
	...
	public static void setupFBO(Framebuffer fbo) {
		...
		EXTFramebufferObject.glRenderbufferStorageEXT('\u8d41', '\u84f9', Minecraft.getMinecraft().displayWidth,
