...
public class KeepSprint extends Module {
    ...
    public void onUpdate(UpdateEvent e) {
        if (!MovementUtils.isMoving() || mc.getPlayer().isSneaking() ||
                (blindness.get() && mc.getPlayer().isPotionActive(Potion.getBlindness())) ||
                (food.get() && !(mc.getPlayer().getFoodStats().getFoodLevel() > 6.0F || mc.getPlayer().getCapabilities().isAllowFlying()))
