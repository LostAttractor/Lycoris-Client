...
import rbq.lycoris.client.event.MouseOverPostEvent
import rbq.lycoris.client.event.Render3DEvent
import rbq.lycoris.client.transformer.ClassTransformer
import rbq.lycoris.client.wrapper.wrappers.render.EntityRenderer
        ...
        for (method in classNode.methods) {
            if (method.name == EntityRenderer.renderWorldPass.name && method.desc == "(IFJ)V") {
                ...
                insnList.add(
                    MethodInsnNode(
                        ...
                        Type.getInternalName(Render3DEvent::class.java),
                        "<init>",
                        "(F)V",
                        false
                    )
            ...
            if (method.name == EntityRenderer.getMouseOver.name) {
                ...
                insnList.add(
                    MethodInsnNode(
                        Opcodes.INVOKESPECIAL,
                        Type.getInternalName(MouseOverPostEvent::class.java),
                        "<init>",
                        "(F)V",
                        false
                    )
